...
process.on('message', function(msg) {
    ...
    if (msg.cmd === 'stop') {
        ...
        .then(function(program) {
            ...
            program.events.on('view:mark', function(event) {
                send({
                    type: 'data', data: {
                        type: 'mark',
                        data: event.data,
                        view_id: event.channel
                    }
                });
            });

            program.events.on('view:tick', function(event) {
                send({
                    type: 'data', data: {
                        type: 'tick',
                        data: event.data,
            ...
            program.events.on('view:eof', function(event) {
                send({
                    type: 'data', data: {
                        type: 'view_end',
                        view_id: event.channel
            ...
            program.events.on('view:points', function(event) {
                send({
                    type: 'data', data: {
                        type: 'points',
                        data: event.data,
