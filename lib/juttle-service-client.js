...
// Initialized in handle_command
let http_url = undefined;
...
function subscribe(opts) {
    ...
    if (opts.observer !== undefined) {
        ...
        ws = new WebSocket(ws_url + '/observers/' + opts.observer);
        ...
        ws = new WebSocket(ws_url + '/jobs/' + opts.job);
        ws.on('message', function(message) {
            write_output(sprintf('received: %s', message));
            let msg = JSON.parse(message);
            if (msg.type === 'ping') {
...
function delete_job(opts) {
    if (opts.job !== undefined) {
        rp({method: 'DELETE', uri: http_url + '/jobs/' + opts.job})
            .then(function(body) {
                write_output(JSON.stringify(JSON.parse(body), null, 2));
            }).error(function(e) {
                write_error(sprintf('ERROR %s', e.message));
...
function command(server, opts, command) {
    ...
    if (! _.has(commands, command)) {
        ...
        return commands[command].perform(opts);
    }
}
